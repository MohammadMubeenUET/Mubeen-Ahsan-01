...
const FMEAEditor: React.FC<FMEAEditorProps> = ({ onBack, initialData }) => {
  ...
  useEffect(() => {
    if (initialData) {
      // Ensure rows have status if loaded from old data
      const enrichedRows = initialData.rows.map(r => ({
          ...r,
          status: r.status || 'Open'
      }));
      setCurrentSystem({...initialData, rows: enrichedRows});
  ...
  const exportCSV = () => {
    if(!currentSystem) return;
    const headers = "Process Step,Failure Mode,Effect,Severity,Cause,Occurrence,Controls,Detection,RPN,Actions,Responsibility,Status\n";
    const rows = currentSystem.rows.map(r => 
      `"${r.processStep}","${r.failureMode}","${r.failureEffect}",${r.severity},"${r.cause}",${r.occurrence},"${r.controls}",${r.detection},${r.rpn},"${r.actions}","${r.responsibility}","${r.status}"`
    ).join("\n");
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex flex-col font-sans">
      ...
      <div className="flex-grow p-4 overflow-auto">
         {currentSystem && (
           <div className="bg-white rounded-lg shadow-xl border border-slate-300 min-w-[1600px]">
              ...
              <table className="w-full text-sm text-left border-collapse">
                 <thead className="bg-slate-800 text-white sticky top-0 z-10">
                    <tr>
                       ...
                       <th className="p-3 border-r border-slate-600 w-32">Status</th>
                    ...
                    {currentSystem.rows.map((row, idx) => (
                        <tr key={row.id} className="border-b border-slate-200 hover:bg-slate-50 group align-top">
                           ...
                           <td className="p-2 border-r border-slate-200 align-middle">
                              <select 
                                 className={`w-full p-1 rounded text-xs font-bold border outline-none ${getStatusColor(row.status)}`}
