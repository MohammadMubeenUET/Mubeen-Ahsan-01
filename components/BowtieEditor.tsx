...
import { 
  ...
  ShieldCheck, Plus, Minus, Lock, KeyRound
} from 'lucide-react';
...
const BowtieEditor: React.FC<BowtieEditorProps> = ({ onBack, initialData, user }) => {
  ...
  // Auth handler for locked features
  const handleLockedFeature = (action: () => void) => {
    if (user) {
        action();
    } else {
        alert("Restricted Feature. Please Sign In from the Home page to access AI tools.");
    }
  };
              ...
              <div className="bg-slate-50 p-6 rounded-xl border border-slate-200 space-y-4">
                ...
                <div className="flex flex-wrap gap-3 pt-2">
                  <button onClick={() => handleAI('normal')} disabled={isGenerating} className="flex-1 py-2 px-4 bg-gradient-to-r from-amber-500 to-orange-600 text-white font-bold rounded-lg flex items-center justify-center gap-2 hover:opacity-90 disabled:opacity-50 transition-opacity">
                    {isGenerating ? <Loader2 className="animate-spin" size={16} /> : <Sparkles size={16} />} AI-Flash Filling
                  </button>
                  <button onClick={() => handleLockedFeature(() => handleAI('rigorous'))} disabled={isGenerating} className="flex-1 py-2 px-4 bg-gradient-to-r from-violet-600 to-purple-700 text-white font-bold rounded-lg flex items-center justify-center gap-2 hover:opacity-90 disabled:opacity-50 transition-opacity">
                    {isGenerating ? <Loader2 className="animate-spin" size={16} /> : <><Lock size={14} className={`text-violet-100 ${user ? 'hidden' : ''}`} /> <BrainCircuit size={16} /></>} Detailed AI-Analysis
                  </button>
                  <button onClick={() => handleLockedFeature(() => setShowHazopInput(!showHazopInput))} className="flex-1 py-2 px-4 bg-cyan-700 text-white font-bold rounded-lg flex items-center justify-center gap-2 hover:bg-cyan-800 transition-colors">
                    <Lock size={14} className={`text-cyan-100 ${user ? 'hidden' : ''}`} /> <FileJson size={16} /> Import HAZOP
                  </button>
                </div>
